...
export const WithdrawScreen: FC = (): JSX.Element => {
    ...
    const handleWithdraw = async (amount: BigNumber): Promise<void> => {
        try {
            ...
            await Promise.all([refetchTotalPooled(), refetchAccountPooled()]);
        } catch (e) {
            handleServerError(
                `Error during withdrawal, ${JSON.stringify(e, null, 2)}`
            );
        }
    };
    ...
    return (
        <ScreenLayout title="Withdraw REN">
            <AmountForm
                ...
                btnVariant="danger"
                disabled={
                    disabled || isAccountLocked || isWrongChain || isLocked
                }
