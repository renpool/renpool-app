...
import {
    DarknoneUrlForm,
    DarknodeParams,
} from "../../components/DarknodeUrlForm";
...
export const AdminScreen: FC = (): JSX.Element => {
    ...
    const { isAllowed, checkForAllowance } = useRenAllowance(
        renPool?.address,
    ...
    const handleApprove = async (): Promise<void> => {
        try {
            ...
            handleServerError(
                `Error during bond approval, ${JSON.stringify(e, null, 2)}`
            );
        }
    };
    ...
    const handleRegister = async ({
        ...
        try {
            const tx = await renPool.registerDarknode(
                darknodeIDBase58ToHex(darknodeID),
                publicKey,
                { gasLimit: 2000000 }
            );
            await tx.wait(); // wait for mining
            ...
            handleServerError(
                `Error during darknode registration, ${JSON.stringify(
                    e,
                    null,
                    2
                )}`
    ...
    const handleEpoch = async (): Promise<void> => {
        try {
            ...
            handleServerError(
                `Error during epoch, ${JSON.stringify(e, null, 2)}`
            );
        }
    };
    ...
    const handleDeregister = async (): Promise<void> => {
        try {
            ...
            handleServerError(
                `Error during darknode deregistration, ${JSON.stringify(
                    e,
                    null,
                    2
                )}`
    ...
    const handleRefund = async (): Promise<void> => {
        try {
            ...
            handleServerError(
                `Error during bond refund, ${JSON.stringify(e, null, 2)}`
            );
        }
    };
    ...
    const handleUnlock = async (): Promise<void> => {
        try {
            ...
            handleServerError(
                `Error during pool unlocking, ${JSON.stringify(e, null, 2)}`
            );
        }
    };
    ...
    return (
        <ScreenLayout title="Admin Panel">
            {!isLocked && (
                <>
                    <Flash my={3} variant="warning">
                        Pool needs to be locked for the darknode to be
                        registered
                    </Flash>
                    <Box p={2} />
                </>
            ...
            <Text>
                Register your darknode by pasting the darknode URL in the box
                below
            </Text>
            <Box p={2} />
            <DarknoneUrlForm
                btnLabel={
                    !isAllowed ? "Approve registration" : "Register darknode"
                }
                btnVariant={!isAllowed ? "default" : "success"}
                disabled={
                    disabled || !isLocked || isAccountLocked || isWrongChain
                }
            ...
            />
            <Box p={4} />
            <Button onClick={handleEpoch} width={1}>
                Epoch
            </Button>
            <Box p={2} />
            <Button onClick={handleDeregister} width={1}>
                Deregister
            </Button>
            <Box p={2} />
            <Button onClick={handleRefund} width={1}>
                Refund
            </Button>
            <Box p={2} />
            <Button onClick={handleUnlock} width={1}>
                Unlock pool
            </Button>
        </ScreenLayout>
    );
