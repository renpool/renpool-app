...
export const RenPoolProvider: FC = ({ children }) => {
    ...
    const [totalPooled, setTotalPooled] = useState<BigNumber>(
        BigNumber.from(0)
    );
    const [isLocked, setIsLocked] = useState<boolean>(false);
    const [accountPooled, setAccountPooled] = useState<BigNumber>(
        BigNumber.from(0)
    );
    ...
    const getNodeOperator = async (): Promise<void> => {
        ...
        try {
            const _nodeOperator: string = await renPool.nodeOperator({
                gasLimit: 60000,
            });
            setNodeOperator(_nodeOperator);
        } catch (e) {
            console.log(
                `Error querying nodeOperator ${JSON.stringify(e, null, 2)}`
            );
        }
    };
    ...
    const getTotalPooled = async (): Promise<void> => {
        ...
        try {
            const _totalPooled: BigNumber = await renPool.totalPooled({
                gasLimit: 60000,
            });
            setTotalPooled(BigNumber.from(_totalPooled));
        } catch (e) {
            console.log(
                `Error querying totalPooled ${JSON.stringify(e, null, 2)}`
            );
        }
    };
    ...
    const getIsLocked = async (): Promise<void> => {
        ...
        try {
            const _isLocked: boolean = await renPool.isLocked({
                gasLimit: 60000,
            });
            setIsLocked(_isLocked);
        } catch (e) {
            console.log(
                `Error querying isLocked ${JSON.stringify(e, null, 2)}`
            );
        }
    };
    ...
    const getBalanceOf = async (): Promise<void> => {
        ...
        try {
            const _pooled: BigNumber = await renPool.balanceOf(account, {
                gasLimit: 60000,
            });
            setAccountPooled(_pooled);
        } catch (e) {
            console.log(
                `Error querying balanceOf ${JSON.stringify(e, null, 2)}`
            );
        }
    };
